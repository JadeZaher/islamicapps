...
# Regex patterns
BAB_PATTERN = re.compile(r'^باب\s+(.+)', re.UNICODE)

# Hadith chain starters — the text uses ( حدثني ) or حدثني or حدثنا
HADITH_START_PATTERN = re.compile(
    r'^\(?\s*حدث[نيتـ]',  # حدثني, حدثنا, حدثتي (OCR variant)
    re.UNICODE
)

# Additional hadith start patterns (sub-narrations and follow-ups)
HADITH_CONTINUATION_PATTERNS = [
    re.compile(r'^و\s*حدث', re.UNICODE),   # وحدثني (and he narrated)
    re.compile(r'^و\s*سأل', re.UNICODE),   # وسألت (and I asked)
...
def parse_text_file(filepath: str) -> list[dict]:
    ...
    # Only process content after the introduction
    ...
    for line_idx, line in enumerate(content_lines, start=CONTENT_START_LINE):
        ...
        if bab_match:
            ...
            current_chapter = f"باب {bab_match.group(1).strip()}"
        ...
        if in_footnote:
            # If the line doesn't look like hadith continuation, skip it
            # Footnotes often span multiple lines
            if not HADITH_START_PATTERN.match(stripped) and not BAB_PATTERN.match(stripped):
                # Heuristic: short lines without Arabic hadith markers are likely
                # still footnote content
                if len(stripped) < 100 and not any(m in stripped for m in ['عن أبيه', 'عن جده', 'زيد بن علي']):
                    continue
                else:
                    in_footnote = False
