...
/**
 * ⚠️ DEPRECATED — superseded by `src/scripts/regen-isnad-graph.ts`
 * ================================================================
 *
 * Track neo4j_isnad_graph_regen_20260516, decision D-1: this importer is
 * retained in place (reversible) as the reference pattern, but is NO LONGER
 * the canonical ingestion path. Do NOT run `npm run db:import` for graph
 * regeneration — use `npm run db:regen` instead.
 *
 * Two production bugs live in this file and are the reason it is deprecated:
 *   1. Narrator duplication — MERGEs narrators on a fresh `uuidv4()` per run
 *      (see `id: uuidv4()` at the Narrator MERGE), so every re-run duplicates
 *      every narrator. The fix MERGEs on the stable `scholar_indx` key.
 *   2. Unified-narrator fallacy — writes a scalar `n.reliability` node
 *      property (from `mapGradeToTabaqah`), collapsing tradition-specific
 *      assessment onto one node. Guardrail G-1 forbids any scalar
 *      `reliability`/`grade` on `:Narrator`/`:Hadith`.
 *
 * `regen-isnad-graph.ts` routes all writes through
 * `src/lib/db/neo4j-helpers.ts` (idempotent MERGE on business keys) and
 * never writes a scalar assessment. Keep this file for historical reference
 * only.
 */
